 ...
 * @brief
 */
export module prism.core:util.optional;

import std;
import :type_traits.basic;
import :type_traits.comparison;
import :util.hashing;
...
namespace prism
{
    ...
    class Optional
    {
      public:
        ...
        {
            return has_value() ? std::addressof(value_) : nullptr;
        }
...
template <typename T>
constexpr bool std::ranges::enable_view<prism::Optional<T>> = true;

template <typename T>
constexpr bool std::ranges::enable_borrowed_range<prism::Optional<T &>> = true;

template <typename T>
constexpr auto std::format_kind<prism::Optional<T>> = std::range_format::disabled;

template <prism::Hashable T>
    requires(!std::is_reference_v<T>)
struct std::hash<prism::Optional<T>>
{
    std::size_t operator()(const prism::Optional<T> &optional) const
