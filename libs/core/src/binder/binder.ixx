...
namespace prism
{
    ...
    class Binder : NonCopyable
    {
      ...
      public:
        ...
        [[nodiscard]] BoundPtr<BoundStatement> bind_statement(const StatementSyntax &syntax,
                                                              const TypeSymbol &return_type,
                                                              const LookupContext &context) const;

        [[nodiscard]] inline BoundPtr<BoundExpression> bind_expression(const ExpressionSyntax &syntax,
                                                                       const LookupContext &context) const
        {
            return bind_expression(syntax, nullptr, context);
        }

        [[nodiscard]] inline BoundPtr<BoundExpression> bind_expression(const ExpressionSyntax &node,
      ...
      private:
        [[nodiscard]] BoundPtr<BoundExpression> bind_expression(const ExpressionSyntax &syntax,
                                                                const TypeSymbol *target_type,
                                                                const LookupContext &context) const;
      ...
      private:
        ...
        [[nodiscard]] BoundPtr<BoundBlock> bind_block(const BlockSyntax &syntax,
                                                      const TypeSymbol &return_type,
                                                      const LookupContext &context) const;
        [[nodiscard]] BoundPtr<BoundVariableDeclaration> bind_variable_declaration_statement(
            const VariableDeclarationStatementSyntax &syntax,
            const LookupContext &context) const;
        [[nodiscard]] BoundPtr<BoundExpressionStatement> bind_expression_statement(
            const ExpressionStatementSyntax &syntax,
            const LookupContext &context) const;
        [[nodiscard]] BoundPtr<BoundReturnStatement> bind_return_statement(const ReturnStatementSyntax &syntax,
                                                                           const TypeSymbol &return_type,
                                                                           const LookupContext &context) const;

        [[nodiscard]] BoundPtr<BoundLiteral> bind_literal_expression(const LiteralExpressionSyntax &syntax,
                                                                     const TypeSymbol *return_type,
                                                                     const LookupContext &context) const;
        [[nodiscard]] BoundPtr<BoundExpression> bind_identifier_expression(const IdentifierExpressionSyntax &syntax,
                                                                           const LookupContext &context) const;
        [[nodiscard]] BoundPtr<BoundBinaryExpression> bind_binary_expression(const BinaryExpressionSyntax &syntax,
                                                                             const LookupContext &context) const;
        [[nodiscard]] BoundPtr<BoundAssignmentExpression> bind_assignment_expression(
            const AssignmentExpressionSyntax &syntax,
            const LookupContext &context) const;
        [[nodiscard]] BoundPtr<BoundExpression> bind_prefix_expression(const PrefixExpressionSyntax &syntax,
                                                                       const TypeSymbol *return_type,
                                                                       const LookupContext &context) const;
        [[nodiscard]] BoundPtr<BoundUnaryExpression> bind_postfix_expression(const PostfixExpressionSyntax &syntax,
                                                                             const LookupContext &context) const;
        [[nodiscard]] BoundPtr<BoundConditionalExpression> bind_ternary_expression(
            const TernaryExpressionSyntax &syntax,
            const TypeSymbol *target_type,
            const LookupContext &context) const;
        [[nodiscard]] BoundPtr<BoundInvocationExpression> bind_invocation_expression(
            const InvocationExpressionSyntax &syntax,
            const LookupContext &context) const;

        [[nodiscard]] BoundPtr<BoundExpression> add_conversion_if_necessary(const ExpressionSyntax &syntax,
                                                                            BoundPtr<BoundExpression> expression,
                                                                            const TypeSymbol &type,
                                                                            const LookupContext &context) const;

        [[nodiscard]] static BoundPtr<BoundExpression> add_conversion_if_necessary(const ExpressionSyntax &syntax,
                                                                                   BoundPtr<BoundExpression> expression,
                                                                                   const TypeSymbol &type,
        ...
        [[nodiscard]] BoundPtr<BoundUnaryExpression> create_unary_operation(const ExpressionSyntax &syntax,
