...
namespace prism
{
    GreenPtr<GreenToken> get_static_green_token(const SyntaxKind kind)
    {
        ...
        if (is_keyword(kind))
        {
            static std::array punctuation_list = {
                ...
                make_ref_counted<GreenToken>(SyntaxKind::close_bracket_token),
                make_ref_counted<GreenToken>(SyntaxKind::open_paren_token),
                make_ref_counted<GreenToken>(SyntaxKind::close_paren_token),
                make_ref_counted<GreenToken>(SyntaxKind::open_brace_token),
                make_ref_counted<GreenToken>(SyntaxKind::close_brace_token),
                ...
                make_ref_counted<GreenToken>(SyntaxKind::bang_token),
                make_ref_counted<GreenToken>(SyntaxKind::bang_equal_token),
                ...
                make_ref_counted<GreenToken>(SyntaxKind::double_colon_token),
            ...
            return punctuation_list[std::to_underlying(kind) - std::to_underlying(SyntaxKind::punctuation_start)];
        }

        return nullptr;
    }

    constexpr Optional<SyntaxKind> match_keyword(const std::string_view text)
    {
        switch (text.size())
        {
            case 2:
    ...
    constexpr Optional<SyntaxKind> match_punctuation(TextCursor &cursor)
    {
        switch (cursor.current())
        {
            case '!':
                ...
                {
                    cursor.advance();
                    return SyntaxKind::bang_equal_token;
                }
                return SyntaxKind::bang_token;
            case '%':
                cursor.advance();
                if (cursor.current() == '=')
                {
            ...
            case ':':
                ...
                {
                    cursor.advance();
                    return SyntaxKind::double_colon_token;
                }
            ...
            case '?':
                ...
                switch (cursor.current())
                {
                    ...
                    case '?':
                        ...
                        {
                            cursor.advance();
                            return SyntaxKind::question_question_equal_token;
                        }
