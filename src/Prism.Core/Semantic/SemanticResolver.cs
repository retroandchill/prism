...
internal sealed class SemanticResolver(SemanticModel model, SemanticBinder binder)
{
    private readonly ConcurrentDictionary<
        ValueSymbol,
        AsyncResolutionState<TypeSymbol>
    > _valueTypeSlots = new(ReferenceEqualityComparer.Instance);
    private readonly ConcurrentDictionary<
        VariableSymbol,
        AsyncResolutionState<BoundExpression>
    > _variableInitializers = new(ReferenceEqualityComparer.Instance);

    private readonly ConcurrentDictionary<
        CallableSymbol,
        AsyncResolutionState<TypeSymbol>
    > _functionReturnTypes = new(ReferenceEqualityComparer.Instance);

    public ResolvedSemanticState Complete()
    {
        return new ResolvedSemanticState
        {
            ValueTypes = _valueTypeSlots.ToFrozenDictionary(
                kvp => kvp.Key,
                kvp => kvp.Value.Result
            ),
            Initializers = _variableInitializers.ToFrozenDictionary(
                kvp => kvp.Key,
                kvp => kvp.Value.Result
            ),
        };
    ...
    {
        return _valueTypeSlots.TryGetValue(valueSymbol, out var slot) && slot.IsCompleted
            ? slot.Result
            : null;
    }

    public ValueTask<TypeSymbol> ResolveValueTypeAsync(
        ValueSymbol symbol,
    ...
    )
    {
        ...
        var slot = _valueTypeSlots.GetOrAdd(
            symbol,
            static _ => new AsyncResolutionState<TypeSymbol>()
        );

        return new ValueTask<TypeSymbol>(
            slot.GetOrStart(() =>
                ResolveValueTypeCoreAsync(
                    symbol,
                    context with
                    {
                        Resolution = context.Resolution.Push(key),
                    },
                    cancellationToken
                )
            )
    ...
    private async Task<TypeSymbol> ResolveValueTypeCoreAsync(
        ValueSymbol symbol,
        BindingContext context,
    ...
    )
    {
        ...
        {
            VariableSymbol variable => await ResolveVariableValueTypeAsync(
                variable,
                context,
                cancellationToken
            ),
            ParameterSymbol parameter => await ResolveParameterValueTypeAsync(
                parameter,
                cancellationToken
            ),
    ...
    private async ValueTask<TypeSymbol> ResolveVariableValueTypeAsync(
    ...
    )
    {
        ...
        var boundInitializer = await ResolveVariableInitializerAsync(
            symbol,
            context,
            cancellationToken
        );
    ...
    private ValueTask<TypeSymbol> ResolveParameterValueTypeAsync(
    ...
    private ValueTask<TypeSymbol> ResolveExplicitTypeAsync(
    ...
    )
    {
        ...
        {
            BuiltInTypeSyntax builtIn => ValueTask.FromResult<TypeSymbol>(
                model.BuiltInTypes.GetSymbol(builtIn.Type)
            ),
    ...
    public ValueTask<BoundExpression?> ResolveVariableInitializerAsync(
        VariableSymbol symbol,
        BindingContext context,
        CancellationToken cancellationToken = default
    )
    {
        ...
        var slot = _variableInitializers.GetOrAdd(
            symbol,
            static _ => new AsyncResolutionState<BoundExpression>()
        );
        return new ValueTask<BoundExpression?>(
            slot.GetOrStart(async () =>
            {
    ...
    public ValueTask<TypeSymbol> ResolveFunctionReturnTypeAsync(
    ...
    )
    {
        ...
        var slot = _functionReturnTypes.GetOrAdd(
            symbol,
            static _ => new AsyncResolutionState<TypeSymbol>()
        );
    ...
    private async Task<TypeSymbol> ResolveReturnTypeCoreAsync(
        CallableSymbol symbol,
        BindingContext context,
        CancellationToken cancellationToken
    )
