...
using Humanizer;
using Prism.SourceGenerator.Metadata;
using Prism.SourceGenerator.Models.CSharp;
using Prism.SourceGenerator.Models.Resolved;
using Riok.Mapperly.Abstractions;
using Riok.Mapperly.Abstractions.ReferenceHandling;

namespace Prism.SourceGenerator.Mappers;
...
public static partial class CSharpModelMapper
{
    ...
    {
        return GetPascalizedName(source);
    }
    ...
    {
        return $"Green{GetPascalizedName(name)}";
    }
    ...
    private static string GetRedCSharpName(string name)
    {
        return name switch
        {
            ...
            _ => $"{GetPascalizedName(name)}Syntax",
        };
    }
    ...
    {
        if (name.StartsWith("isize", StringComparison.OrdinalIgnoreCase))
        {
            return $"ISize{name[5..].Pascalize()}";
        }

        if (name.StartsWith("usize", StringComparison.OrdinalIgnoreCase))
        {
            return $"USize{name[5..].Pascalize()}";
        }
