...
using ZLinq;
using static Prism.SourceGenerator.Metadata.CommonNames;

namespace Prism.SourceGenerator.Emitters;
    ...
    extension(CodeWriter writer)
    {
        ...
        public void EmitSyntaxKinds(CSharpSyntaxModel model)
        {
            ...
            using (writer.EnterBlockScope())
            {
                ...
                writer.WriteLine("public ushort AsUnderlyingType() => (ushort)kind;");
                writer.WriteLine();
        ...
        private void WriteTrie(TrieNode node)
        {
            switch (node.Children.Count)
            {
                case 1:
                {
                    var (character, child) = node.Children.AsValueEnumerable().Single();
                ...
                case > 1:
                {
                    ...
                    foreach (
                        var (character, child) in node
                            .Children.AsValueEnumerable()
                            .OrderBy(x => x.Key)
                    )
                    {
        ...
        private void EmitDiagnosticDescriptorConstant(CSharpDiagnostic diagnostic)
        {
            ...
            using var scope = writer.EnterBlockScope(true);
            writer.WriteLine($"Id = nameof(DiagnosticCode.{diagnostic.CSharpName}),");
